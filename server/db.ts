...
export async function submitLead(email: string): Promise<InsertLead> {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const result = await db.insert(leads).values({ email }).onDuplicateKeyUpdate({
      set: { email },
    });
    return { email };
  } catch (error) {
    console.error("[Database] Failed to submit lead:", error);
    throw error;
  }
}
